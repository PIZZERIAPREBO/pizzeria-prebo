    ...
    def create_payment_withholdings(self, payment_group):
        for tax in self.filtered(lambda x: x.withholding_type != 'none'):
            ...
            if payment_withholding:
                ...
                """
                Determinamos si el impuesto a calcular es ISLR o IVA
                segun el tipo de retencion indicado en la configuracion del impuesto
                para ello dirigase a:
                    withholding taxes -> withholding computation -> type
                    - tabla islr: ISLR
                    - alicuota en partner: IVA

                Despues usamos el withholding objetivo para ubicar el diario a usar
                para configurarlo:
                    journals -> payment method types -> seleccione withholding -> impuesto a retener
                """
                ...
                journal = self.env['account.journal'].search([
                    ...
                    ('withholding_target', '=', 'income'),
                    ('type', 'in', ['cash', 'bank']),
                ], limit=1) if tax.withholding_type == 'tabla_islr' else \
                    self.env['account.journal'].search([
                    ('company_id', '=', tax.company_id.id),
                    ('outbound_payment_method_ids', '=', payment_method.id),
                    ('at_least_one_inbound', '!=', 'at_least_one_outbound'),
                    ('withholding_target', '=', 'value'),
                    ('type', 'in', ['cash', 'bank']),
                ], limit=1)
